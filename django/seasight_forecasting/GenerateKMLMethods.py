...
def GenerateHistoricKML(region, dateFrom, check, dateTo, alt, lat, lon):
    ...
    data.nlon = pd.to_numeric(data.lon, downcast="float")
    data.nlat = pd.to_numeric(data.lat, downcast="float")
    data.analysed_sst = pd.to_numeric(data.sst, downcast="float")
    ...
    for group in data.groupby(['time']):
        time = group[1]['time']
        ngroup = group[1].drop(['time'], axis=1)
        ngroup = GetClusters(n_clusters, ngroup)
        ngroup['time'] = time
        regions.append(GetRegions(n_clusters, ngroup, InitCmap(data['sst']), True))
